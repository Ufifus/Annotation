...
//        request.open('POST', '/proj/add');
//        request.send(data);
        $http.post("proj/add", data)
    ...
    let $dialog = $('<div></div>').html('SplitText').dialog({
        ...
        buttons: {
            "Delete Project": function () {
                ...
                let run_url = "proj/" + projectName + "/delete";
                $dialog.dialog('close');
                xhr.onreadystatechange = function () {
                    $dialog.dialog('close');
                    if (xhr.readyState == 4) {
                        let table = document.getElementById("projects-table").getElementsByTagName('tbody')[0];
                        let rowIndex =document.querySelector("#projectRow_"+projectid).rowIndex;
