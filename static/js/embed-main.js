...
function handleMouseOver(d, i) {  // Add interactivity
    ...
    if (embedName.includes("roi")) {
        img_url = new URL('proj/' + proj_name + '/roi/' +
            embedName, window.location.origin);
            console.log(img_url);
    } else {
        img_url = new URL('proj/' + proj_name + '/get_embed/' +
            embedName, window.location.origin);
            console.log(img_url);
    }
...
function handleClick(d, i) {
    ...
    imageId = fileName.slice(-1)
    // we will load the annotation page for this image:
    let new_url = "proj/" + proj_name + '/image/' + imageId;
...
function reloadChartData() {
    let run_url = new URL(embed_csvUrl, window.location.origin);
    console.log(run_url);
...
function make_embed() {
    ...
    let run_url = new URL(embed_for_projUrl, window.location.origin);
